	...
	real M_prefactor=-1/(2*N[i]*pow(M[i],0.5));
		...
		real3 Fjcomp=make_real3(0);
			...
			real rik_pow5=pow((rik_norm-CUTOFF)/1,5);
                        ...
                        real rdot = rij.x*rik.x + rij.y*rik.y + rij.z*rik.z/(rij_norm*rij_norm);
                        real P6=(231*pow(rdot,6.0)-315*pow(rdot,4.0)+105*pow(rdot,2.0)-5)/16;
                        real delta_P6=(1386*pow(rdot,5.0)-1260*pow(rdot,3)+210*rdot)/16;
                        Ficomp += delta_switch_ij*switch_ik*P6*delta_rij_norm + switch_ij*delta_switch_ik*P6*delta_rik_norm + switch_ij*switch_ik*delta_P6*delta_rij_norm;//this very last term is not quite right
                        Fjcomp += -delta_switch_ij*switch_ik*P6*delta_rij_norm + switch_ij*switch_ik*delta_P6*delta_rij_norm;
                        real3 Fkcomp = -switch_ij*delta_switch_ik*P6*delta_rik_norm + switch_ij*switch_ik*delta_P6*delta_rij_norm;
